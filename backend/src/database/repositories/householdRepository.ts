...
class HouseholdRepository {
  ...
  static async create(data, options: IRepositoryOptions) {
    ...
    await record.setStudents(data.students || [], {
      transaction,
    });
  ...
  static async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.familyLastName) {
        whereAnd.push(
          SequelizeFilterUtils.ilike(
            'household',
            'familyLastName',
            filter.familyLastName,
          ),
        );
      }
      ...
      if (filter.primaryGuardianFullName) {
        whereAnd.push(
          SequelizeFilterUtils.ilike(
            'household',
            'primaryGuardianFullName',
            filter.primaryGuardianFullName,
          ),
        );
      }

      if (filter.primaryGuardianCellPhone) {
        whereAnd.push(
          SequelizeFilterUtils.ilike(
            'household',
            'primaryGuardianCellPhone',
            filter.primaryGuardianCellPhone,
          ),
        );
      }
  ...
  static async findAllAutocomplete(query, limit, options: IRepositoryOptions) {
    ...
    const records = await options.database.household.findAll(
      {
        attributes: ['id', 'primaryGuardianFullName'],
        where,
        limit: limit ? Number(limit) : undefined,
        orderBy: [['primaryGuardianFullName', 'ASC']],
      },
    );

    return records.map((record) => ({
      id: record.id,
      label: record.primaryGuardianFullName,
    }));
  }
  ...
  static async _fillWithRelationsAndFiles(record, options: IRepositoryOptions) {
    ...
    output.students = await record.getStudents({
      transaction,
    });
